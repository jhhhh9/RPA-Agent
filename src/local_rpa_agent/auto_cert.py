...
def load_auto_cert_rows(context: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    package_keyword = str(context.get("package_file_keyword") or "label_update")
    ...
    package_img = find_package_img(label_dir, package_keyword)
...
def find_package_img(root: Path, keyword: str) -> str:
    if not root.is_dir():
        return ""
    for path in root.rglob("*"):
